...
export default function Dashboard() {
    ...
    useEffect(() => {
        ...
        data.then((user) => {
            ...
            if (user) {
                ...
                if (user.user.id) {
                    fetchDoctorData(user.user.id).then((data) => {
                        setImage(data.image);
    ...
    return (
        <main className="bg-[#246BFD] flex">
            ...
            <div className="w-80%">
                <MainSection firstName={doctorData?.firstName!} lastName={doctorData?.lastName!} email={doctorData?.email!} image={image as any} />
            </div>
        </main>
    )
}